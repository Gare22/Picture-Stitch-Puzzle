...
func setup(source_image: Texture2D, p_columns: int, p_rows: int) -> void:
	...
	var board_scale := minf(avail_w / img_w, avail_h / img_h)

	# Display size for the whole board
	var total_board_w := img_w * board_scale
	var total_board_h := img_h * board_scale

	# Scale factor from image pixels to screen pixels
	var display_scale := total_board_w / img_w
	...
	for row in range(rows_count):
		for col in range(columns_count):
			...
			# Integer pixel boundaries — no gaps, no overlaps
			...
			# Display size derived from the integer region (not a shared float)
			var disp_w := region_w * display_scale
			var disp_h := region_h * display_scale
			...
			piece.custom_minimum_size = Vector2(disp_w, disp_h)
			piece.size = piece.custom_minimum_size

			# Position manually — no GridContainer rounding issues
			piece.position = Vector2(col * disp_w, row * disp_h)
	...
	# Add children in shuffled order.
	for i in range(pieces.size()):
		add_child(pieces[grid_indices[i]])

	# Size this control to match the board
...
func swap_pieces(piece_a, piece_b) -> void:
	...
	var pos_b := -1
	for i in range(all_pieces.size()):
		if all_pieces[i] == piece_a:
			pos_a = i
		if all_pieces[i] == piece_b:
			pos_b = i
